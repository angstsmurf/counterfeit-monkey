...
Understand "search for [text]" as text-searching when a switched on computer which is running a search engine is visible. Text-searching is an action applying to one topic.

Carry out text-searching the topic understood (this is the divert searching to input reply rule):
	if a switched on computer which is running a search engine (called the software chosen) is visible:
...
To deliver (chosen message - text) to (chosen program - an email program):
	repeat through the message table of the chosen program:
		if message entry is chosen message: 
			...
			if the chosen program is visible:
				say "[new mail message of the chosen program][paragraph break]";

Understand "read [text]" as email-reading when a switched on unprotected computer which is running an email program is visible. Email-reading is an action applying to one topic.

Carry out email-reading the topic understood (this is the divert email reading to input reply rule):
	if a switched on unprotected computer which is running an email program (called the chosen software) is visible:
...
Understand "reply to [text]" or "answer [text]" as email-replying when a computer which is running an email program is visible. Email-replying is an action applying to one topic.
